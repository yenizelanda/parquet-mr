...
public class TestGroupVarInt {
	...
	private void control(int[] ints) throws IOException {
		...
		GroupVarIntValuesWriter gvw = new GroupVarIntValuesWriter(50);

		for (int i : ints) {
			gvw.writeInteger(i);
		}

		BytesInput bi = gvw.getBytes();
		GroupVarIntValuesReader gvr = new GroupVarIntValuesReader();

		gvr.initFromPage(ints.length, bi.toByteArray(), 0);
