 ...
 */
public class VariableWidthRunLengthBitPackingHybridEncoder {
  private static final Log LOG = Log.getLog(VariableWidthRunLengthBitPackingHybridEncoder.class);

  private BytePacker packer;

  private int byteCap;
  private byte packerWidth;
  ...
  public VariableWidthRunLengthBitPackingHybridEncoder(int initialCapacity) {
    if (DEBUG) {
      LOG.debug(String.format("Encoding: RunLengthBitPackingHybridEncoder with initialCapacity %d", initialCapacity));
    }


    this.baos = new CapacityByteArrayOutputStream(initialCapacity);
    this.packBuffer = new byte[32];
  ...
  private void writeOrAppendBitPackedRun() throws IOException {
    ...
    byteCap = 0;
    for (int i = 0; i < 8; i++)
    	byteCap = byteCap | bufferedValues[i];
    
    packerWidth = ((byte) (32 - Integer.numberOfLeadingZeros(byteCap)));
    
    packer = ByteBitPackingLE.getPacker(packerWidth);
    
    packer.pack8Values(bufferedValues, 0, packBuffer, 0);
    
    baos.write(packerWidth);
    
    baos.write(packBuffer, 0, packerWidth);
  ...
  private void writeRleRun() throws IOException {
    ...
    BytesUtils.writeIntLittleEndian(baos, previousValue);
